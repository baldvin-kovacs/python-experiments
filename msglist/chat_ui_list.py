...
class ChatUIList(Gtk.ApplicationWindow):
    ...
    def setup_ui(self) -> None:
        ...
        self.selection_model = Gtk.NoSelection(model=self.message_model)
        self.message_list_view.set_model(self.selection_model)

        # Connect to model changes to auto-scroll
        self.message_model.connect("items-changed", self.on_model_items_changed)

        # Track when we need to scroll
        ...
        self.past_scroll.set_child(self.message_list_view)

        # Connect to the scroll adjustment to know when to scroll
        adj = self.past_scroll.get_vadjustment()
        adj.connect("changed", self.on_scroll_changed)

        main_box.append(self.past_scroll)
    ...
    def setup_css(self) -> None:
        ...
        css = """
        /* Disable ListView row hover effects */
        listview > row {
            background: none;
        }
    ...
    def on_factory_setup(
        ...
        # Create message box container that fills the full width
        message_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=0)
        message_box.set_hexpand(True)
        message_box.get_style_context().add_class("message-container")
        ...
        list_item.set_child(message_box)
    ...
    def on_factory_bind(
        ...
        message_box = list_item.get_child()
    ...
    def on_model_items_changed(self, model, position, removed, added):
        """Handle model changes and auto-scroll to bottom when items are added."""
        if added > 0:  # Only scroll when items are added
            self._should_scroll = True

    def on_scroll_changed(self, adjustment):
        """Handle scroll adjustment changes and scroll to bottom if needed."""
    ...
    def simulate_typing(self) -> None:
        ...
        def typing_simulation() -> None:
            while True:
                ...
                if len(self.typing_messages) < 3 and self.past_message_index < len(
                    self.msgs
                ):
                ...
                if self.typing_messages:
                    ...
                    full_message = self.msgs[
                        self.past_message_index - len(self.typing_messages) - 1
                    ]
                    user_num = (len(self.typing_messages) % 3) + 1

                    GLib.idle_add(
                        self.add_past_message, f"User {user_num}", full_message
                    )
                    GLib.idle_add(self.update_typing_messages)
